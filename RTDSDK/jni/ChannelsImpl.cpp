  ...
  (JNIEnv *env, jobject obj, jstring friendlyName, jstring uniqueName, jstring attribute, jint type, jlong nativeChannelsContext) {
	...
	if (nativeChannelsContext == 0) {
		...
		if (channelsPtr != nullptr) {
			...
			if(friendlyName != NULL) {
				nativeFriendlyNameString = env->GetStringUTFChars(friendlyName, JNI_FALSE);
				channelPtr->setFriendlyName(nativeFriendlyNameString, NULL);
				env->ReleaseStringUTFChars(friendlyName, nativeFriendlyNameString);
			}
